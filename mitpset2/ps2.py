...
#
# Answer:
# The graph in this problem represent blocks on MIT campus
# The edges represent the distance between the blocks
# The distance are represented in the form of weighted edges
...
def load_map(map_filename):
    ...
    with open(map_filename) as file:
        for line in file:
            ...
            mapp.add_node(src)
            mapp.add_edge(dest)
            MagEdges = WeightedEdge(src,dest,int(Tdis),int(Odis))
            mapp.add_edge(MagEdges)
    return mapp        
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    for node in digraph.get_edges_for_node(start):
        if node not in path:
            total_dis += node.get_total_distance()
            total_oud_dis += node.get_outdoor_distance()
            if shortest == None or len(path)<=len(shortest):
                new_path = directed_dfs(digraph,node,end,max_total_dist,max_dist_outdoors)
                if new_path != None:
                    shortest = new_path
    if total_dis <= max_total_dist and total_oud_dis<= max_dist_outdoors:
        return shortest
